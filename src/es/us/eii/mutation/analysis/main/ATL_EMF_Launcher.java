...
public class ATL_EMF_Launcher {
	...
	public static void executeATLTransformation(String executionsCanPath, String folderTransformations, String IN_MM, String OUT_MM, String Trace_MM, 
			String IN_MM_NAME, String OUT_MM_NAME, String Trace_MM_NAME, String Trace_OutModel_Name, String	transformationName, int numModels, int numMutants){
		try {
			/* Initializations */
			...
			for (int i = 1; i<= numModels; i++){
				for (int j=1; j<=numMutants; j++) {
					/* Run transformation */
					//Inject source model
					IModel sourceModel = modelFactory.newModel(sourceMM);
					injector.inject(sourceModel, executionsCanPath + "/" + MODELS_FOLDER + "/" + INDIVIDUAL_MODEL_FOLDER + i + "/AutomaticModel_" + IN_MM.substring(0, IN_MM.length()-6) + i + ".xmi");
					...
					transformationLauncher.launch(ILauncher.RUN_MODE, new NullProgressMonitor(), parameters,
						new FileInputStream(executionsCanPath + "/" + folderTransformations + "/" + transformationName + "_Mutant" + j + ".asm"));
					//Extract the output model and trace model
					extractor.extract(targetModel, executionsCanPath + "/" + MODELS_FOLDER + "/" + INDIVIDUAL_MODEL_FOLDER + i + "/" + OUT_MM.substring(0, OUT_MM.length()-6) + "_out" + i + "_Mutant" + j +".xmi");
					extractor.extract(traceModel, executionsCanPath +  "/" + MODELS_FOLDER + "/" + INDIVIDUAL_MODEL_FOLDER + i + "/" + Trace_OutModel_Name + i + "_Mutant" + j + ".xmi");
	...
	public static void executeATLTransformation2(String executionsCanPath, String folderTransformations, String IN_MM1, String IN_MM2, String OUT_MM,  
			String IN_MM_NAME1, String IN_MM_NAME2, String OUT_MM_NAME, String	transformationName, int numModels, int numMutants){
		try {
			/* Initializations */
			ILauncher transformationLauncher = new EMFVMLauncher();
			ModelFactory modelFactory = new EMFModelFactory();
			...
			for (int i = 1; i<= numModels; i++){
				for (int j=1; j<=numMutants; j++) {
					...
					injector.inject(sourceModel1, executionsCanPath + "/" + MODELS_FOLDER + "/" + INDIVIDUAL_MODEL_FOLDER + i + "/AutomaticModel_" + IN_MM1.substring(0, IN_MM1.length()-6) + i + ".xmi");
					IModel sourceModel2 = modelFactory.newModel(sourceMM2);
					injector.inject(sourceModel2, executionsCanPath + "/" + MODELS_FOLDER + "/" + INDIVIDUAL_MODEL_FOLDER + i + "/" + IN_MM2.substring(0, IN_MM2.length()-6) + "_out" + i + "_Mutant" + j +".xmi");
					//Define target model and trace model
					IModel targetModel = modelFactory.newModel(targetMM);			
					...
					extractor.extract(targetModel, executionsCanPath + "/" + MODELS_FOLDER + "/" + INDIVIDUAL_MODEL_FOLDER + i + "/" + "JointModel" + i + "_Mutant" + j + ".xmi");
